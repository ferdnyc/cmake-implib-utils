...
IMPORTED_LOCATION and IMPORTED_IMPLIB, as well as the properties
IMPORTED_LOCATION_<CONFIG> and IMPORTED_IMPLIB_<CONFIG> for each
...
Currently there is only one public API call here:
  ...
  fix_implib(TARGET target [PACKAGE_NAME package])
...
``fix_implib`` will attempt to locate the corresponding ``.dll`` file
...
the ``IMPORTED_IMPLIB[_<CONFIG>]`` property where it belongs.
...
from the target name. So, ``fix_implib(TARGET My::lib)`` will set the
cache variable ``Mylib_RUNTIME_DLL[_<CONFIG>]``.

Because it operates on already-defined targets, ``fix_implib()`` can
...
configure targets on Windows systems. In many cases ``fix_implib`` can
correct the ``IMPORTED_`` paths of targets produced by those modules.)

Dependencies
^^^^^^^^^^^^

``fix_implib`` relies on the ``dlltool.exe`` utility. The path can
be set by defining ``DLLTOOL_EXECUTABLE`` in the cache prior to
including this module, if it is not set implib_utils will attempt to
...
function(find_dll _outvar _implib _cachevar)
...
### Function: fix_implib(TARGET target [PACKAGE_NAME package])
#
# (Win32 only)
...
# The import library path is moved to IMPORTED_IMPLIB instead.
#
# Required arguments:
#   TARGET target
...
#   (e.g. ``fix_implib(TARGET My::lib)`` will create Mylib_RUNTIME_DLL
#   in the cache, if it finds a DLL to match the IMPORTED_LOCATION.)
macro(fix_implib)
  set(options)
  set(oneValueArgs TARGET PACKAGE_NAME)
  set(multiValueArgs)
  cmake_parse_arguments(FindImpLib "" "TARGET;PACKAGE_NAME" "" ${ARGN})

  if(NOT DEFINED FindImpLib_TARGET OR NOT TARGET ${FindImpLib_TARGET})
    message(SEND_ERROR
      "fix_implib() not called with a valid target argument: '${FindImpLib_TARGET}'")
    return()
  endif()
  if(DEFINED FindImpLib_PACKAGE_NAME)
    set(_cachevar "${FindImpLib_PACKAGE_NAME}_RUNTIME_DLL")
  else()
    ...
    string(REPLACE ":" "" _cachevar "${FindImpLib_TARGET}_RUNTIME_DLL")
  endif()

  # Look up which configs we (might) have to find DLLs for, beyond
  # the anonymous default one that we'll always check
  set(_suffixes ";")
  get_target_property(_cfgs ${FindImpLib_TARGET} IMPORTED_CONFIGURATIONS)
  ...
  foreach(_sfx IN LISTS _suffixes)
    get_target_property(_lib ${FindImpLib_TARGET} IMPORTED_LOCATION${_sfx})
    get_target_property(_implib ${FindImpLib_TARGET} IMPORTED_IMPLIB${_sfx})
    message(DEBUG "${FindImpLib_TARGET} IMPORTED_LOCATION${_sfx}: '${_lib}'")
    message(DEBUG "${FindImpLib_TARGET} IMPORTED_IMPLIB${_sfx}: '${_implib}'")
    if("${_lib}" MATCHES "\\.dll$")
    ...
    if("${_lib}" STREQUAL "_lib-NOTFOUND")
      ...
      if("${_sfx}" STREQUAL "" AND ${_no_type_suffixes})
        message(SEND_ERROR
          "Target ${FindImpLib_TARGET} properties incorrectly defined!")
      endif()
      continue()
    endif()
    if(NOT "${_lib}" MATCHES "(\\.lib|\\.dll\\.a)$")
      message(SEND_ERROR
        "Target ${FindImpLib_TARGET} property IMPORTED_LOCATION${_sfx} "
    ...
    unset(_lib)
    if(EXISTS "${_implib}")
      find_dll(_dll "${_implib}" "${_cachevar}${_sfx}")
      ...
      message(DEBUG "Set ${FindImpLib_TARGET} IMPORTED_LOCATION${_sfx} to ${_dll}}")
    endif()
  endforeach()
endmacro()
